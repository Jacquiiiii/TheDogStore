export const productsData = {
  productsData: [
    {
      id: 1,
      name: "Red Meat, Liver & Whole Oats Recipe Dog Food",
      description: "A delicious, nourishing kibble that was specially crafted to help support a shelter dogs transition from the adoption environment to their forever home.",
      brand: "Acana",
      image: "https://www.acana.com/dw/image/v2/BFDW_PRD/on/demandware.static/-/Sites-cpf-master-catalog/default/dw0eb73872/images/dogstar/ACANA%20Rescue%20Dog%20Red%20Meat%20Front%20Right%2022.5lb%20USA.png?sw=450",
      price: 44.25,
      category: "Food"
    },
    {
      id: 5,
      name: "Bully stick",
      description: "Chew Time Beef Pizzles are made from quality, European raised cattle. Made only of one ingredient, the beef pizzle makes for the perfect all natural, healthy, low fat, high protein, and a 100% digestible dog chew/treat alternative that will have your dog drooling for more.",
      brand: "Chew Time",
      image: "https://www.petonly.ca/media/catalog/product/cache/49039c4fcdf92ce8cf0ef4ed79baf63e/1/8/188-01006.png",
      price: 9.05,
      category: "Treats"
    },
    {
      id: 12,
      name: "Squeaky Plush Dog Toy",
      description: "This adorable plush dog toy is made with soft, durable material and contains a squeaker for added entertainment.",
      brand: "KONG",
      image: "https://cdn.shopify.com/s/files/1/0529/8693/7544/products/631_CHAR_840px_360x.png?v=1631621515,%20https://cdn.shopify.com/s/files/1/0529/8693/7544/products/631_CHAR_840px_480x.png?v=1631621515%20480w",
      price: 8.52,
      category: "Toys"
    },
    {
      id: 13,
      name: "Marble Dog Bowl",
      description: "A standard bowl meant for drinking or eating. Available in two sizes; Small and Medium. Dishwasher safe. Do not microwave. Multiple colors. Happy times all around.",
      brand: "Top Paw",
      image: "https://cdn.shopify.com/s/files/1/0575/2507/6173/products/Marble-Bowl_White_Large-min-957x1024_1200x1200.png?v=1623753944",
      price: 6.59,
      category: "Supplies"
    }
  ]
}

export const foodData = {
  productsByCategory: [
    {
      id: 1,
      name: "Red Meat, Liver & Whole Oats Recipe Dog Food",
      description: "A delicious, nourishing kibble that was specially crafted to help support a shelter dogs transition from the adoption environment to their forever home.",
      brand: "Acana",
      image: "https://www.acana.com/dw/image/v2/BFDW_PRD/on/demandware.static/-/Sites-cpf-master-catalog/default/dw0eb73872/images/dogstar/ACANA%20Rescue%20Dog%20Red%20Meat%20Front%20Right%2022.5lb%20USA.png?sw=450",
      price: 44.25,
      category: "Food"
    },
    {
      id: 2,
      name: "Red Meat, Liver & Whole Oats Recipe Dog Food",
      description: "A delicious, nourishing kibble that was specially crafted to help support a shelter dogs transition from the adoption environment to their forever home.",
      brand: "Acana",
      image: "https://www.acana.com/dw/image/v2/BFDW_PRD/on/demandware.static/-/Sites-cpf-master-catalog/default/dw0eb73872/images/dogstar/ACANA%20Rescue%20Dog%20Red%20Meat%20Front%20Right%2022.5lb%20USA.png?sw=450",
      price: 44.25,
      category: "Food"
    },
    {
      id: 3,
      name: "Red Meat, Liver & Whole Oats Recipe Dog Food",
      description: "A delicious, nourishing kibble that was specially crafted to help support a shelter dogs transition from the adoption environment to their forever home.",
      brand: "Acana",
      image: "https://www.acana.com/dw/image/v2/BFDW_PRD/on/demandware.static/-/Sites-cpf-master-catalog/default/dw0eb73872/images/dogstar/ACANA%20Rescue%20Dog%20Red%20Meat%20Front%20Right%2022.5lb%20USA.png?sw=450",
      price: 44.25,
      category: "Food"
    },
    {
      id: 4,
      name: "Red Meat, Liver & Whole Oats Recipe Dog Food",
      description: "A delicious, nourishing kibble that was specially crafted to help support a shelter dogs transition from the adoption environment to their forever home.",
      brand: "Acana",
      image: "https://www.acana.com/dw/image/v2/BFDW_PRD/on/demandware.static/-/Sites-cpf-master-catalog/default/dw0eb73872/images/dogstar/ACANA%20Rescue%20Dog%20Red%20Meat%20Front%20Right%2022.5lb%20USA.png?sw=450",
      price: 44.25,
      category: "Food"
    }
  ]
}

export const suppliesData = {
  productsByCategory: [
    {
      id: 13,
      name: "Marble Dog Bowl",
      description: "A standard bowl meant for drinking or eating. Available in two sizes; Small and Medium. Dishwasher safe. Do not microwave. Multiple colors. Happy times all around.",
      brand: "Top Paw",
      image: "https://cdn.shopify.com/s/files/1/0575/2507/6173/products/Marble-Bowl_White_Large-min-957x1024_1200x1200.png?v=1623753944",
      price: 6.59,
      category: "Supplies"
    },
    {
      id: 14,
      name: "Marble Dog Bowl",
      description: "A standard bowl meant for drinking or eating. Available in two sizes; Small and Medium. Dishwasher safe. Do not microwave. Multiple colors. Happy times all around.",
      brand: "Top Paw",
      image: "https://cdn.shopify.com/s/files/1/0575/2507/6173/products/Marble-Bowl_White_Large-min-957x1024_1200x1200.png?v=1623753944",
      price: 6.59,
      category: "Supplies"
    },
    {
      id: 15,
      name: "Marble Dog Bowl",
      description: "A standard bowl meant for drinking or eating. Available in two sizes; Small and Medium. Dishwasher safe. Do not microwave. Multiple colors. Happy times all around.",
      brand: "Top Paw",
      image: "https://cdn.shopify.com/s/files/1/0575/2507/6173/products/Marble-Bowl_White_Large-min-957x1024_1200x1200.png?v=1623753944",
      price: 6.59,
      category: "Supplies"
    },
    {
      id: 16,
      name: "Marble Dog Bowl",
      description: "A standard bowl meant for drinking or eating. Available in two sizes; Small and Medium. Dishwasher safe. Do not microwave. Multiple colors. Happy times all around.",
      brand: "Top Paw",
      image: "https://cdn.shopify.com/s/files/1/0575/2507/6173/products/Marble-Bowl_White_Large-min-957x1024_1200x1200.png?v=1623753944",
      price: 6.59,
      category: "Supplies"
    }
  ]
}

export const toysData = {
  productsByCategory: [
    {
      id: 9,
      name: "Squeaky Plush Dog Toy",
      description: "This adorable plush dog toy is made with soft, durable material and contains a squeaker for added entertainment.",
      brand: "KONG",
      image: "https://cdn.shopify.com/s/files/1/0529/8693/7544/products/631_CHAR_840px_360x.png?v=1631621515,%20https://cdn.shopify.com/s/files/1/0529/8693/7544/products/631_CHAR_840px_480x.png?v=1631621515%20480w",
      price: 8.52,
      category: "Toys"
    },
    {
      id: 10,
      name: "Squeaky Plush Dog Toy",
      description: "This adorable plush dog toy is made with soft, durable material and contains a squeaker for added entertainment.",
      brand: "KONG",
      image: "https://cdn.shopify.com/s/files/1/0529/8693/7544/products/631_CHAR_840px_360x.png?v=1631621515,%20https://cdn.shopify.com/s/files/1/0529/8693/7544/products/631_CHAR_840px_480x.png?v=1631621515%20480w",
      price: 8.52,
      category: "Toys"
    },
    {
      id: 11,
      name: "Squeaky Plush Dog Toy",
      description: "This adorable plush dog toy is made with soft, durable material and contains a squeaker for added entertainment.",
      brand: "KONG",
      image: "https://cdn.shopify.com/s/files/1/0529/8693/7544/products/631_CHAR_840px_360x.png?v=1631621515,%20https://cdn.shopify.com/s/files/1/0529/8693/7544/products/631_CHAR_840px_480x.png?v=1631621515%20480w",
      price: 8.52,
      category: "Toys"
    },
    {
      id: 12,
      name: "Squeaky Plush Dog Toy",
      description: "This adorable plush dog toy is made with soft, durable material and contains a squeaker for added entertainment.",
      brand: "KONG",
      image: "https://cdn.shopify.com/s/files/1/0529/8693/7544/products/631_CHAR_840px_360x.png?v=1631621515,%20https://cdn.shopify.com/s/files/1/0529/8693/7544/products/631_CHAR_840px_480x.png?v=1631621515%20480w",
      price: 8.52,
      category: "Toys"
    }
  ]
}

export const treatsData = {
  productsByCategory: [
    {
      id: 5,
      name: "Bully stick",
      description: "Chew Time Beef Pizzles are made from quality, European raised cattle. Made only of one ingredient, the beef pizzle makes for the perfect all natural, healthy, low fat, high protein, and a 100% digestible dog chew/treat alternative that will have your dog drooling for more.",
      brand: "Chew Time",
      image: "https://www.petonly.ca/media/catalog/product/cache/49039c4fcdf92ce8cf0ef4ed79baf63e/1/8/188-01006.png",
      price: 9.05,
      category: "Treats"
    },
    {
      id: 6,
      name: "Bully stick",
      description: "Chew Time Beef Pizzles are made from quality, European raised cattle. Made only of one ingredient, the beef pizzle makes for the perfect all natural, healthy, low fat, high protein, and a 100% digestible dog chew/treat alternative that will have your dog drooling for more.",
      brand: "Chew Time",
      image: "https://www.petonly.ca/media/catalog/product/cache/49039c4fcdf92ce8cf0ef4ed79baf63e/1/8/188-01006.png",
      price: 9.05,
      category: "Treats"
    },
    {
      id: 7,
      name: "Bully stick",
      description: "Chew Time Beef Pizzles are made from quality, European raised cattle. Made only of one ingredient, the beef pizzle makes for the perfect all natural, healthy, low fat, high protein, and a 100% digestible dog chew/treat alternative that will have your dog drooling for more.",
      brand: "Chew Time",
      image: "https://www.petonly.ca/media/catalog/product/cache/49039c4fcdf92ce8cf0ef4ed79baf63e/1/8/188-01006.png",
      price: 9.05,
      category: "Treats"
    },
    {
      id: 8,
      name: "Bully stick",
      description: "Chew Time Beef Pizzles are made from quality, European raised cattle. Made only of one ingredient, the beef pizzle makes for the perfect all natural, healthy, low fat, high protein, and a 100% digestible dog chew/treat alternative that will have your dog drooling for more.",
      brand: "Chew Time",
      image: "https://www.petonly.ca/media/catalog/product/cache/49039c4fcdf92ce8cf0ef4ed79baf63e/1/8/188-01006.png",
      price: 9.05,
      category: "Treats"
    }
  ]
}

export const kongBrandData = {
  productsByBrand: [
    {
      id: 9,
      name: "Squeaky Plush Dog Toy",
      description: "This adorable plush dog toy is made with soft, durable material and contains a squeaker for added entertainment.",
      brand: "KONG",
      image: "https://cdn.shopify.com/s/files/1/0529/8693/7544/products/631_CHAR_840px_360x.png?v=1631621515,%20https://cdn.shopify.com/s/files/1/0529/8693/7544/products/631_CHAR_840px_480x.png?v=1631621515%20480w",
      price: 8.52,
      category: "Toys"
    },
    {
      id: 10,
      name: "Squeaky Plush Dog Toy",
      description: "This adorable plush dog toy is made with soft, durable material and contains a squeaker for added entertainment.",
      brand: "KONG",
      image: "https://cdn.shopify.com/s/files/1/0529/8693/7544/products/631_CHAR_840px_360x.png?v=1631621515,%20https://cdn.shopify.com/s/files/1/0529/8693/7544/products/631_CHAR_840px_480x.png?v=1631621515%20480w",
      price: 8.52,
      category: "Toys"
    },
    {
      id: 11,
      name: "Squeaky Plush Dog Toy",
      description: "This adorable plush dog toy is made with soft, durable material and contains a squeaker for added entertainment.",
      brand: "KONG",
      image: "https://cdn.shopify.com/s/files/1/0529/8693/7544/products/631_CHAR_840px_360x.png?v=1631621515,%20https://cdn.shopify.com/s/files/1/0529/8693/7544/products/631_CHAR_840px_480x.png?v=1631621515%20480w",
      price: 8.52,
      category: "Toys"
    },
    {
      id: 12,
      name: "Squeaky Plush Dog Toy",
      description: "This adorable plush dog toy is made with soft, durable material and contains a squeaker for added entertainment.",
      brand: "KONG",
      image: "https://cdn.shopify.com/s/files/1/0529/8693/7544/products/631_CHAR_840px_360x.png?v=1631621515,%20https://cdn.shopify.com/s/files/1/0529/8693/7544/products/631_CHAR_840px_480x.png?v=1631621515%20480w",
      price: 8.52,
      category: "Toys"
    }
  ]
}